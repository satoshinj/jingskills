...
开始前根据用户当前这轮原话选择唯一一级。这张表和判定规则与 `jing` 主入口、`jing-x-article` 完全一致，不在本 Skill 里另作解释：
...
`kind: translation` 或 `kind: repost` 是译介稿，正文属于原作者。除了本轮授权判定，还要读 `source_permission`：只有 `granted` 和 `open-license` 能进后台，`pending` 停在本地并告诉用户缺的是原作者许可，不是脚本问题。契约见 `jing-writer/references/translation.md`。
