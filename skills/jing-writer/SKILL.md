---
name: jing-writer
description: "把灵感、资料或草稿写成中文长文，审阅重写文章，或转成口播稿、实战 thread 与合规译介稿。用于内容写作与改写，不负责公众号排版或在线发布；临时文章不要求先建知识库。"
---
...
用户明确要求公众号排版或保存草稿时，把通过检查的文章、公众号封面和署名偏好交给 `jing-wechat`。它按 [平台授权与确认](../jing/references/platform-authorization.md) 生成并验证本地预览，满足风格确认与草稿写入授权后创建或更新草稿；当前任务已有的有效确认不重复询问。不要在 `jing-writer` 内临时拼 HTML 或直接调用微信接口。
